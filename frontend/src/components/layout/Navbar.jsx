...
import { Bell, User, Menu, Dices } from 'lucide-react';
import { IconButton, NightmareTransition } from '../common';
...
export default function Navbar() {
  ...
  const handleRandomNightmare = () => {
    ...
    // Determine route based on item type (movies vs series)
    // For simplicity, checking if it has seasons to identify series
    const route = randomItem.seasons ? `/series/${randomItem.slug}` : `/movies/${randomItem.slug}`;
    
    setTimeout(() => {
      navigate(route);
      setTimeout(() => setIsNightmareActive(false), 500); // give it time to unmount
    }, 400); // 400ms flash before nav
  };

  return (
    <>
      <NightmareTransition isActive={isNightmareActive} />
      <header 
        className={clsx(
        ...
        <div className="container mx-auto px-6 lg:px-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            <IconButton 
              icon={Dices} 
              variant="ghost" 
              className="text-crimson hover:text-crimson-bright hover:bg-crimson/10" 
              aria-label="Random Nightmare" 
              onClick={handleRandomNightmare}
            />
            <NavSearch />
